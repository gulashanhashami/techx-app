...
const webcamRef = React.useRef(null);
  ...
  const deleteEditImage=(imageItem, imageIndex)=>{
    // for(var i=0;i<handleDeleteImage.length;i++){
    //      if(handleDeleteImage[i]===imageItem){
      console.log("hello")
  ...
  return (
        ...
        <div className='editCameraImage'>
         {(openCamera) ? (<>
        <Webcam videoConstraints={videoConstraints} ref={webcamRef} screenshotFormat="image/jpeg" className='editSetCamera'/>
        <button onClick={()=>{
          capture();
           setOpenCamera(false);
    ...
    <div className='editContain1'>
        ...
        <div className='editTextBox'>
            <p style={{ marginLeft:"7%", color:"blue", fontSize:"2.5vw", fontWeight:"800"}}>Edit Post</p>
        ...
        <input name='textData' className='editwriteYourText' type="text" defaultValue={editdataItem.postText} placeholder="What's in your mind?" onChange={(e)=>setPostText(e.target.value)} />
        </div>
        ...
        {handleDeleteImage.map((imageItem, imageIndex) => {
                  return (
                    <div key={imageIndex} className="editpostedImageBox">
                      ...
                      <p style={{fontSize:"1.7vw", fontWeight:"bold", color:"gray"}}>Media {imageIndex+1}</p>
                      </div>
                      <span style={{ marginRight:"7%", color:"blue", fontSize:"2.5vw", cursor:"pointer"}}><p onClick={()=>{deleteEditImage(imageItem,imageIndex)}}>X</p></span>
                    </div>
                  );
                })}
        </div>
        ...
        <input name='photoImage' className='editinputForImage' id="file-input" type="file" multiple="true" onChange={(e)=>handleChangeForFile(e)} />
        </label>
         ...
         <label>
            <div onClick={()=>{ setOpenCamera(!openCamera)}} className='edituploadFile'>
        ...
        <Button onClick={handleDeleteFunctionInEditpage} variant="contained" className='editButtonBox' style={{fontSize:"1.5vw",  borderRadius: "1vw", backgroundColor:"white", color:"red", border:"2px solid red"}}>Delete</Button>
        <Button onClick={postEditedDataFunction} variant="contained" className='editButtonBox' style={{fontSize:"1.5vw",  borderRadius: "1vw"}}>Update</Button>
        </div>
    </div>
</div>
</>
